 ...
 */
  ...
  void AGG_posicion::mutar(tSolution &gen) {
    set<size_t> posibles;
  ...
  ResultMH AGG_posicion::optimize(Problem *problem, int maxevals){
    assert(maxevals > 0);
    size_t evals = 0;
    ...
    for (; evals < maxevals; evals += 50) {
        ...
        while (cruces < n_cruces) {
          for (; index<poblacion_sel.size() && cruces < n_cruces; index+=2) {
              cruce_intercambio(poblacion_sel[index], poblacion_sel[index+1], poblacion_nueva);
              cruces ++;
          }
        }
